...
set(gca,'xlim',[-50 400],'yscale','log','ytick',logspace(log10(min_freq),log10(max_freq),6),'yticklabel',round(logspace(log10(min_freq),log10(max_freq),6)*10)/10)
xlabel('Time (ms)');
ylabel('Frequency *Hz');
...
%% CANDECOMP
addpath('C:\Users\hirok\Documents\MATLAB\nway320');

model = parafac(eegpower_trials, 3, [0 0 0 10 1000], [2 2 2]);
...
figure(5); clf;
contourf(EEG.times,frex,frequencies(:,3)*times(:,3)',40,'linecolor','none');
set(gca,'xlim',[-50 400],'yscale','log','ytick',logspace(log10(min_freq),log10(max_freq),6),'yticklabel',round(logspace(log10(min_freq),log10(max_freq),6)*10)/10)
...
labels(standardTrials) = 1;
labels(oddballTrials) = 2;
figure(6); clf;
boxplot(trials(:,2), labels)