...
% We use the srcipt acquired from the EEGLAB wiki.
% https://eeglab.org/tutorials/09_source/EEG_sources.html

%% Fitting dipoles to independent components:
...
EEG = pop_dipfit_settings( EEG, ...
    'hdmfile', template_models(2).hdmfile, ...
    'coordformat', template_models(2).coordformat,...
...
figure; 
pop_topoplot(TMPEEG, 0, 1, ...
    [ 'ERP 100ms, fit with a single dipole (RV ' num2str(dipole(1).rv*100,2) '%)'], 0, 1);
set(gcf, 'color', 'w');